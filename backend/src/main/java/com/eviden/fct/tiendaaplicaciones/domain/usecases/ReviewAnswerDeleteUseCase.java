...
@Component
public class ReviewAnswerDeleteUseCase {
	
	private final AuthenticationApplicationService authService;
	private final ReviewDomainService reviewService;

	public Review invoke(Long reviewId) throws UnauthorizedException, ForbiddenException, NotFoundException, ConflictException {
		...
		return reviewService.removeReviewResponse(reviewId);
	}
